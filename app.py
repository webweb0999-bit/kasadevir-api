...
API_KEY = os.environ.get("API_KEY", "kasadevir_2024_EK_gizli_anahtar")
...
def setup_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS vardiyalar (
            id SERIAL PRIMARY KEY, vardiya_no TEXT NOT NULL,
            tarih TEXT NOT NULL, acilis_zaman TEXT DEFAULT '',
            kapanis_zaman TEXT, devreden TEXT DEFAULT '',
            devralan TEXT DEFAULT '', devir_nakit REAL DEFAULT 0,
            toplam_gider REAL DEFAULT 0, sayilan_nakit REAL,
            peron_nakit REAL DEFAULT 0, beklenen_nakit REAL,
            fark REAL, durum TEXT DEFAULT 'acik'
        );
        CREATE TABLE IF NOT EXISTS giderler (
            id SERIAL PRIMARY KEY, vardiya_id INTEGER NOT NULL,
            zaman TEXT NOT NULL, kategori TEXT NOT NULL,
            tutar REAL NOT NULL, aciklama TEXT DEFAULT ''
        );
        CREATE TABLE IF NOT EXISTS calisanlar (
            id SERIAL PRIMARY KEY, isim TEXT NOT NULL UNIQUE
        );
        CREATE TABLE IF NOT EXISTS urun_talepleri (
            id SERIAL PRIMARY KEY, zaman TEXT NOT NULL,
            urun_adi TEXT NOT NULL, soran TEXT DEFAULT '',
            not_metni TEXT DEFAULT '', durum TEXT DEFAULT 'bekliyor'
        );
        CREATE TABLE IF NOT EXISTS islem_logu (
            id SERIAL PRIMARY KEY, zaman TEXT NOT NULL,
            islem TEXT NOT NULL, detay TEXT DEFAULT ''
        );
    """)
...
def vardiya_ekle():
    ...
    try:
        ...
        cur.execute("""
            ...
            (vardiya_no,tarih,acilis_zaman,devreden,devralan,
             devir_nakit,toplam_gider,durum,peron_nakit)
...
def vardiya_getir(vid):
    ...
    try:
        ...
        row = row_to_dict(cur); conn.close()
        return jsonify(row) if row else (jsonify({"error": "Bulunamadi"}), 404)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def vardiya_guncelle(vid):
    ...
    try:
        d = request.json; conn = get_db(); cur = conn.cursor()
        alanlar = ", ".join([k + "=%s" for k in d.keys()])
        cur.execute("UPDATE vardiyalar SET " + alanlar + " WHERE id=%s",
                    list(d.values()) + [vid])
        conn.commit(); conn.close()
...
def gider_ekle():
    ...
    try:
        ...
        yeni_id = row_to_dict(cur)["id"]
        cur.execute("""UPDATE vardiyalar SET toplam_gider=
            (SELECT COALESCE(SUM(tutar),0) FROM giderler WHERE vardiya_id=%s)
            WHERE id=%s""", (d["vardiya_id"], d["vardiya_id"]))
        conn.commit(); conn.close()
        return jsonify({"id": yeni_id}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def gider_guncelle(gid):
    ...
    try:
        d = request.json; conn = get_db(); cur = conn.cursor()
        alanlar = ", ".join([k + "=%s" for k in d.keys()])
        cur.execute("UPDATE giderler SET " + alanlar + " WHERE id=%s",
                    list(d.values()) + [gid])
        conn.commit(); conn.close()
        return jsonify({"ok": True})
    except Exception as e:
...
def urun_ekle():
    ...
    try:
        d = request.json; conn = get_db(); cur = conn.cursor()
        cur.execute("""INSERT INTO urun_talepleri (zaman,urun_adi,soran,not_metni)
